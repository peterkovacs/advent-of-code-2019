  ...
  func visible(from: Coordinate) -> [Coordinate] {
    ...
    while !queue.isEmpty {
      ...
      case .asteroid:
        ...
        from.line(to: pos, limitedBy: endIndex.x).forEach {
            if case .asteroid = grid[$0] { encounteredAsteroid = true }
            if encounteredAsteroid { grid[$0] = .scanned(.unknown) }
        }
      }

      queue.append(contentsOf: pos.neighbors8(limitedBy: endIndex.x, and: endIndex.y).filter { if case .scanned(_) = grid[$0] { return false } else { return true } } )
    }
...
let grid = Grid( square: stdin.joined().map { $0 == "#" ? State.asteroid : State.empty } )
